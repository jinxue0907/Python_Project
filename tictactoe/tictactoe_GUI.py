...
class TictactoeGUI:
    ...
    def init_GUI(self):
        ...
        self.images = {}    #{'X': PhotoImage객체, '0': PhotoImage객체}
        self.images['X'] = tkinter.PhotoImage(file='X.gif')
        self.images['O'] = tkinter.PhotoImage(file='O.gif')

        self.canvas.bind('<Button-1>', self.click_handler)  #*** click_handler에 괄호가 없다, 지금 실행하는 것이 아니다
        self.root.mainloop()
    ...
    def draw_board(self):
        ...
        for i, v in enumerate(self.game_engine.board):
            ...
            x += TILE_SIZE
            if i % self.game_engine.SIZE == self.game_engine.SIZE - 1:  #다음 줄
                x = 0
                y += TILE_SIZE
